# Final Project Documentation - Umbotella
...
In `odrive_command.py`, we have calculated the velocity of left and right wheels.
```python
Vl = (v + self.wheel_track * w / 2 ) / self.R
Vr = (v - self.wheel_track * w / 2 ) / self.R
```

In `keymapping_node.py`, we have changed the maximum linear and angular speed to give the robot a more steady move.
```python
self.max_linear_speed = 5
self.max_angular_speed = 8
```
